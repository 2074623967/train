...
export default defineComponent({
  ...
  setup(props, { emit }) {
     ...
     */
    const queryAllTrain = () => {
      const list = window.SessionStorage.get(window.SESSION_ALL_TRAIN);
      if (window.Tool.isNotEmpty(list)) {
        ...
        axios.get('/business/train/query-all').then(response => {
          ...
          if (data.success) {
            ...
            window.SessionStorage.set(window.SESSION_ALL_TRAIN, trains.value);
          } else {
